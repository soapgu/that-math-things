...
v2.6 是当前正在规划的下一版本，将新增独立于现有加减法“计算训练”的九九乘法闯关。题库覆盖 `1–9 × 1–9` 共 81 道有序乘法题，使用同一张 9×9 乘法坐标表，通过逐级减少可见乘积形成简单、中等、困难三档难度。

本版本支持 10、20、50 题单局闯关、逐题即时反馈，以及由正确率和平均用时共同决定的一至三星评价。首版不保存历史成绩，不接入现有 `practice-records`、统计、订正或辅助运算流程。
...
| 4. UI 视觉设计 | 🎯 下一步 | 固化视觉层级、组件状态、响应式和动效规范 | 高保真稿、视觉与组件规范 |
| 5. 技术原型与基础模型 | ⏳ 待开始 | 验证题库、矩阵显示、选项和评分模型 | 纯函数、矩阵原型、单元测试 |
| 6. 纵向功能实现 | ⏳ 待开始 | 按最小闭环、完整难度、体验完善三轮交付 | 可运行的设置—答题—结算流程 |
