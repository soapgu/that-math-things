...
| 6. 纵向功能实现 | 🔄 进行中（第二轮完成） | 按最小闭环、完整难度、体验完善、开发收尾四轮交付 | 可运行的设置—答题—结算流程 |
...
- 下一轮先按原计划完成第三轮“体验完善”，补齐自动切题、离开确认、再来一局、反馈播报和最终动效。
- 第三轮完成后再进入第四轮“开发收尾”：优化提升/挑战难度的格内输入痕迹与结果过渡，并将 v2.5 通用 `useTimer` 改为基于时间戳差值的准确计时。
