...
| 2. UE 页面流程设计 | ✅ 已完成 | 明确设置 Tab、页面关系、背诵状态机、恢复、重置和异常回退 |
| 3. 低保真交互原型 | ✅ 已完成 | 验证两张表、81 格自选、逐句确认、语音操作和支持设备布局 |
| 4. UI 视觉设计 | ✅ 已完成 | 固化组件状态、响应式、动效和无障碍规范 |
| 5. 技术原型与基础模型 | ✅ 已完成 | 验证 45 句、81 格映射、存储和语音生命周期 |
| 6. 纵向功能实现 | ✅ 已完成 | 四轮已完成统一入口、完整顺序背、自定义背与体验收尾 |
| 7. 测试与体验验收 | 🟡 等待体验确认 | 自动化和Chromium验收通过，等待Firefox与用户体验确认 |
...
- 已形成 [v2.7 九九乘法口诀背诵 UE 页面流程设计](./docs/v2.7-九九乘法口诀背诵UE页面流程设计.md)。
- 已锁定统一设置页 Tab、查询参数、开始/继续/重新开始和返回行为。
- 已明确乘法表与口诀表始终同页同步显示：宽屏左右并排、窄空间上下排列，并与逐句确认区共享当前状态。
- 已定义顺序背、自定义选择、模式切换、自动领读、手动确认、恢复与整表完成状态机。
- 已定义离开不丢进度、损坏数据回退、存储失败提示、键盘和减少动态效果规则。
- 已形成 12 项低保真验证清单；步骤 2 完成，下一步进入步骤 3“低保真交互原型”。

### ✅ 步骤 3：低保真交互原型（已完成）

- 已新增独立可点击 HTML 原型，覆盖统一设置、顺序背、自定义背、混合进度、44/45、45/45、语音与异常状态。
- 已验证双表同步显示、逐句确认、交换律双格展开、平方单格、快速确认和键盘单停靠点。
- 三个真实 Chromium 目标视口均无横向溢出或控制台错误；1440 左右并排，768/1024 上下排列。
- 1024 并排候选会将乘法格压到约 40px、口诀文字压到 9px，因此正式选择上下排列。
- 已按体验反馈取消“口诀背诵”页面标题，将返回、模式、当前口诀、语音、确认、进度和重置合并为 48px 高的吸顶单行控制栏；完成状态改为内联提示，双表标题改为边框内小标签。
- `1440×900` 以右侧口诀表为高度基准，将左右面板显式固定为约573px、两个网格固定为约532px，避免内容或缩放造成高度漂移；产品区约760px，1024/768继续上下排列并保留约62px/72px的正方形乘法格。
- 口诀表顶部增加中文“一”至“九”列头，使左右都成为10行；逐行上、下边界误差均为0px，9个表头不进入45句完成统计。
- 分组标题增加派生完成态：组内口诀全部完成后使用与右侧已背口诀格一致的底色和字重，不显示勾选；左侧乘法格继续使用勾选，重置无需额外清理标题数据。
- 已修正Firefox对Grid子项最小高度处理不同而导致的右表撑高：1440面板与网格锁定最小/实际/最大高度，直接子项允许收缩并限制溢出；Chrome自动回归和用户Firefox体验复核均通过。
- 详细证据和体验清单见 [v2.7 九九乘法口诀背诵低保真交互原型](./docs/v2.7-九九乘法口诀背诵低保真交互原型.md)。
- 用户已确认统一入口、单行控制栏、双表同步、10行对齐、分组标题反馈和完成状态；步骤3完成，下一步进入步骤4“UI视觉设计”。

### ✅ 步骤 4：UI 视觉设计（已完成）

- 已新增独立高保真HTML原型，在低保真完整状态机上叠加视觉层。
- 已锁定与v2.6同体系的“安静数学练习纸”方向：品牌蓝表示当前，青绿色表示已背。
- 已定义统一设置页、单行控制栏、两张表、分组标题、语音、重置和45/45完成视觉。
- 左侧乘法格使用青绿底与✓，右侧口诀格和分组标题只使用青绿底与字重。
- 已定义220ms当前句进入、240ms新格展开和680ms整表柔光；减少动态效果直接显示稳定状态。
- 评审视口扩展为768/1024/1440/1920，1920继续使用1400px居中布局。
- 详细令牌、组件规范、自动化证据和Firefox复核清单见 [v2.7 九九乘法口诀背诵 UI 视觉设计](./docs/v2.7-九九乘法口诀背诵UI视觉设计.md)。
- 用户已确认蓝色当前态、青绿色已背态、单行控制栏、双表状态、分组标题、四档响应式和克制柔光符合预期；步骤4完成，下一步进入步骤5“技术原型与基础模型”。

### ✅ 步骤 5：技术原型与基础模型（已完成）

- 已建立45句传统口诀、81格交换律映射、分组完成派生和未背答案安全视图。
- 已实现Schema v1会话规范化、独立本地存储、损坏数据回退和写入失败降级。
- 已实现中文领读控制器，支持取消、旧回调隔离、不可用降级和卸载清理。
- 已确定复用边界：共享坐标能力与页面框架；背诵使用独立路由、状态机和双表组件，原闯关矩阵保持不变。
- 独立React技术原型覆盖顺序、自定义、混合进度、44/45、45/45和四档响应式。
- 全量36个测试文件、353项测试、生产构建和真实Chromium验证通过。
- 详细证据见[v2.7 九九乘法口诀背诵技术原型与基础模型](./docs/v2.7-九九乘法口诀背诵技术原型与基础模型.md)。
- 步骤5完成，下一步进入步骤6“纵向功能实现”。

### ✅ 步骤 6：纵向功能实现（已完成）

- `/multiplication`已升级为闯关/背诵统一设置页，缺失或无效参数保持默认闯关，Tab使用查询参数无历史堆积切换。
- 新增正式`/multiplication/recitation`路由，并仅对该页面放宽现有`AppLayout`内容宽度以容纳1400px双表。
- 已走通“一一得一”自动领读、停止或结束后手动确认、保存进度、展开`1×1`并进入“一二得二”的最小顺序闭环。
- 开始与确认立即写入独立Schema v1；保存失败保留内存进度，无有效会话直接访问时返回背诵设置Tab。
- 设置页和背诵页已共用重新开始确认；取消完整保留会话并恢复焦点，确认后立即从“一一得一”开始新会话。
- 完成第三轮后，全量37个测试文件、381项测试和生产构建通过；既有四档宽屏/窄屏布局约束未改动。
- 第二轮已完成完整45句传统顺序、跨组推进、普通口诀双格展开、平方口诀单格展开和最早未完成恢复。
- 45/45后会话闭合为无当前句状态，口诀表45格、9个分组标题和乘法表81格全部完成；设置页提供“查看完成结果”。
- 第三轮已开放81格自定义选择、点击方向显示、交换律关联、最近模式恢复和顺序/自定义共享进度；模式及选择变化立即保存。
- 第四轮已完成680ms整表柔光、220/240ms局部反馈及减少动态效果降级；恢复既有45/45不会重播柔光。
- 已区分空存储、损坏会话和存储不可访问，写入失败不回滚内存进度；语音异常不阻断手动确认。
- 已补齐顺序句、确认按钮、自定义网格、重置对话框和完成态焦点，并使用独立礼貌状态区播报简短进度。
- 全量37个测试文件、394项测试、生产构建和Diff检查通过；Chromium四档视口无横向溢出，1440/1920双表继续保持573px面板、532px网格与10行对齐。
- 步骤6完成，下一步进入步骤7“测试与体验验收”。

### 🟡 步骤 7：测试与体验验收（自动化通过，等待用户体验确认）

- 新增背诵Page Object及9项专项Chromium E2E，覆盖统一入口、完整45句、自定义方向、模式切换、恢复、重置、异常降级、语义和四档响应式。
- 全量37个测试文件、394项Vitest、13个spec、82项Playwright、生产构建和Diff检查全部通过；既有功能73项E2E无回归。
- `768×1024`、`1024×768`保持上下排列；`1440×900`、`1920×1080`保持1400px内并排、573px面板、532px网格和10行对齐。
- 验收发现并修复P2焦点问题：重置弹窗改为在真实打开完成后聚焦默认保留按钮，关闭完成后恢复触发按钮。
- Chromium可视抽查完成态无横向或纵向滚动，控制台无错误或警告；Firefox人工复核和用户体验清单待确认。
- 详细证据见 [v2.7 九九乘法口诀背诵测试与体验验收](./docs/v2.7-九九乘法口诀背诵测试与体验验收.md)。
- 用户确认前不进入步骤8，不升级版本、不发布或部署。
...
- [docs/v2.7-九九乘法口诀背诵UE页面流程设计.md](./docs/v2.7-九九乘法口诀背诵UE页面流程设计.md)：v2.7 页面流程、背诵状态机与低保真验证输入
- [docs/v2.7-九九乘法口诀背诵低保真交互原型.md](./docs/v2.7-九九乘法口诀背诵低保真交互原型.md)：v2.7 可点击线框、自动化证据与体验确认清单
- [docs/v2.7-九九乘法口诀背诵UI视觉设计.md](./docs/v2.7-九九乘法口诀背诵UI视觉设计.md)：v2.7 视觉令牌、高保真原型与体验确认
- [docs/v2.7-九九乘法口诀背诵技术原型与基础模型.md](./docs/v2.7-九九乘法口诀背诵技术原型与基础模型.md)：v2.7 领域接口、复用边界、技术原型与验证证据
- [docs/v2.7-九九乘法口诀背诵测试与体验验收.md](./docs/v2.7-九九乘法口诀背诵测试与体验验收.md)：v2.7 自动化、浏览器验收证据与人工体验清单
