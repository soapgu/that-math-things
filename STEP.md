...
| 3. 低保真交互原型 | 🎯 下一步 | 验证两张表、81 格自选、逐句确认、语音操作和支持设备布局 |
...
- 为统一设置页、顺序背、自定义背和整表完成绘制关键状态线框。
- 验证双表同步显示、逐句确认和交换律双格展开；重点比较 `1024×768` 的并排与上下布局。
- 在 `768×1024`、`1024×768`、`1440×900` 下验证可读性、触控、键盘和无溢出。
- 记录并关闭 UE 文档中的 12 项验证问题，再进入 UI 视觉设计。
