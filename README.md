...
下一版本：**v2.6 九九乘法闯关（步骤 3：累计开图与 81 题模式待体验确认）**

### 支持设备

| 设备 | 支持状态 | 说明 |
|---|---|---|
| 平板 | ✅ 支持 | 最小宽度 `768px`，覆盖竖屏和横屏 |
| 电脑 | ✅ 支持 | 重点覆盖 `1024px` 和 `1440px` 等常见宽度 |
| 手机小屏 | ❌ 暂不支持 | 宽度低于 `768px` 时显示设备提示，不提供应用功能 |

`767px` 仅作为不支持设备的拦截测试边界，不代表手机端适配。手机小屏将在后续版本单独规划。
...
- 日常开发：`git pull && npm install && npm start`
- 新增题目：参考「开发指南 → 如何新增一道题」的模板，并自行核查
  ① `problems/data/` 新建定义 → ② `registry.js` 注册 → ③ `components/animations/` 动画 → ④ `ProblemDetail` 映射
- Git 工作流：`feat/<name>` 分支 → 合入 main → push
- 部署：`npm run build` 后手动部署 `build/` 目录，或 `npm run deploy` 推送 GitHub Pages

> 阶段性实施记录与验收标准见 [STEP.md](./STEP.md)。
...
**Schema v2**（v2.4 起当前生效）按题聚合为 `items[]`：
...
```js
{
  ...
  items: [
    {
      index: 0,
      question: { a, b, op, answer, hasCarry, hasBorrow },
      userAnswer,
      result: { isCorrect, errors: string[], detail: string|null },
      assistUsage: { eligible, kind, used, level, method, strategy } | null
    }
  ],
  evaluation: {
    difficulty, accuracy, speed,
    composite: { totalStars, grade, comment }
  }
}
```

- `assistUsage.eligible` 题目是否真实进位/退位；false 表普通题不进摘要
- `assistUsage.used === (level > 0)`，保存时强制校验一致
- `level: 0/1/2` 分别表示未使用 / 只看提醒 / 查看方法
- 旧 v1 记录由 `storage.js` 兼容读取层转换，`assistUsage` 置为 `null`（不误判独立完成）

> 完整 schema 演进与字段说明见 [STEP.md](./STEP.md#辅助使用记录)。
> 新增题目与 Phase 实施记录也见 STEP.md。
