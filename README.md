...
| 步骤 | 内容 | 状态 |
|---|---|---|
| 3a | 统计数据页（错误分布统计 + 历史记录排行 + 清除数据） | ✅ |

### v2.1 规划

| 步骤 | 内容 | 状态 |
|---|---|---|
| b | 保存逻辑迁移：Session 保存，Result 纯展示 | ✅ |
| a | 历史记录点击详情（Stats → Result 闭环导航） | ✅ |
| c | 严重错误判定：`markQuestion` 增加严重错误分类，结算页显示 | ✅ |
| d | 订正功能：原题一模一样重做，实时批改直到全对，不写记录/结算，可在历史记录上反复订正 | ✅ |

### v2.2 规划：综合评价体系

#### 概述

每次练习结算时，对本次练习进行三维度星级评价，并展示雷达图 + 综合评语。历史记录点详情时同样可见（复用结算页）。
...
`difficulty = cbWeight × (rangeScore + 2)`
