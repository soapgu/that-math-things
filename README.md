...
- `assistGenerator.js`：判断辅助资格，计算个位、十位中间值、进退位动作和步骤数据。
...
- 实现十位/个位表格、成捆小棒、单根小棒及一个十与十个一之间转换的基础组件。
- 统一进位上箭头、退位下箭头和数位提示条。
...
- 实现进位加法动画：数位对齐、个位相加、个位写数、向十位进 1、十位相加、合并结果。
- 实现退位减法动画：十位退 1、个位增加 10、个位相减、十位相减、合并结果。
