...
- **聚焦难点**：只辅助确实涉及进位或退位、且能够形成有效拆分步骤的题目。
- **由浅入深**：先给进退位提醒，仍然卡住时再展示完整方法。
- **方法与数位同步**：凑十、破十、平十负责说明“怎么算”，进位、退位负责说明“数位发生了什么”，两者在同一时间线中演示。
...
- 进位加法：使用凑十法。
- 退位减法：根据训练设置使用破十法或平十法。
- 存在有意义的拆分过程，能够生成至少两个辅助步骤的题目。

不纳入辅助范围：

- 不进位加法和不退位减法。
- 一步即可直接得到结果、无法形成有效拆分过程的特殊题。
- 乘除法。
- 应用题已有的“辅助解题”模式；本功能仅服务于计算训练。

辅助资格不使用简单的数值阈值判断，而由辅助步骤生成器判断。无法生成有效步骤时，不展示辅助入口。

#### 设置页调整

保留现有 `assistEnabled` 和 `assistMethod` 设置，但调整语义：

- `assistEnabled` 表示本次训练中允许主动求助，不表示每题自动展示分步引导。
- 辅助开关说明改为“做题时可主动查看进位、退位和分步提示”。
- `assistMethod` 只控制退位减法使用破十法还是平十法；进位加法固定使用凑十法。
- 辅助开启后，将方法选项标题明确为“退位减法提示方法”。
...
assistance.eligible &&
(current.hasCarry || current.hasBorrow)
```
...
第二层不是只播放凑十、破十或平十步骤，而是用两条同步轨道展示完整思考过程：
...
1. 高亮个位 `8 + 7`，提示个位相加超过 10，需要进位。
2. 将 7 拆成 2 和 5，展示 `7 = 2 + 5`。
3. 用 2 与个位的 8 凑十，展示 `28 + 2 = 30`。
4. 通过十格框或圆点动画表现“10 个一换成 1 个十”，并提示向十位进 1。
5. 加上剩余的 5，展示 `30 + 5 = 35`。
6. 最后说明“3 个十和 5 个一组成 35”，但仍由孩子自己填写答案。

视觉上使用十格框和圆点：从第二个加数移动一部分圆点填满个位十格框，十个圆点组合成一个“十”并移动到十位区域，再展示剩余圆点。

#### 破十法演示

以 `15 - 8` 为例：

1. 高亮个位 `5` 和减数 `8`，提示 5 不够减 8，需要退位。
2. 将 15 分成 10 和 5。
3. 动画表现从十位退 1：一个“十”拆成 10 个圆点，十位减少 1。
4. 展示 `10 - 8 = 2`，从十个圆点中移走 8 个。
5. 将剩余的 2 与原来的 5 合并，展示 `2 + 5 = 7`。
6. 再次强调“从十位退 1，这个 1 表示 10”。

核心记忆点：拆大数，先从 10 里面减，再把剩下的数加回来。

#### 平十法演示

同样以 `15 - 8` 为例：

1. 提示个位 5 不够减 8，这道题需要退位。
2. 为了先减到整十，将 8 拆成 5 和 3。
3. 展示 `15 - 5 = 10`，先移走 5 个圆点。
4. 将一个“十”拆成 10 个一，提示“1 个十换成 10 个一”。
5. 展示 `10 - 3 = 7`，再移走剩余的 3 个圆点。
6. 强调平十法同样发生了退位，不能省略退位说明。

核心记忆点：拆小数，先减到整十，再减剩下的数。
...
{
  eligible: true,
  kind: 'carry',               // carry | borrow
  method: 'makeTen',           // makeTen | breakTen | flatTen
  reminder: '个位相加超过 10，记得向十位进 1。',
  prompt: '8 还差几可以凑成 10？',
  operands: {
    first: 8,
    second: 7,
  ...
  placeValueAction: {
    ...
    to: 'tens',
    amount: 1,
    message: '10 个一换成 1 个十，向十位进 1'
  },
  steps: [
    { type: 'reminder', text: '个位 8 + 7 超过 10，需要进位' },
    { type: 'split', text: '把 7 分成 2 和 5' },
    { type: 'carry', text: '28 + 2 = 30', placeValueAction: 'onesToTens' },
    { type: 'combine', text: '30 + 5 = 35' }
  ]
}
```

退位题的 `placeValueAction.type` 为 `borrow`，提示从十位退 1，并将 1 个十换成 10 个一。
...
```js
assistUsage: [
  { used: true, level: 2, method: 'makeTen' },
  { used: true, level: 1, method: 'breakTen' },
...
│           ├── CarryBorrowHint.jsx
│           ├── TenFrame.jsx
│           ├── MakeTenAnimation.jsx
│           ├── BreakTenAnimation.jsx
│           └── FlatTenAnimation.jsx
├── utils/
│   ├── assistGenerator.js
...
- `assistGenerator.js`：判断辅助资格，计算拆分数字、进退位动作和步骤数据。
- `MathAssist`：管理未展开、第一层提醒、第二层演示三个状态。
- `TenFrame`：复用十格框、圆点、组合与拆分的基础视觉能力。
- 三个方法动画：只消费步骤数据并负责展示，不重复计算公式。
- `PracticeSession`：提供入口、切题时重置状态、收集辅助使用记录，不承载具体算法。
...
- 完成辅助资格判断，排除不进位、不退位和无有效拆分步骤的题目。
- 实现凑十法、破十法、平十法的拆分算法。
- 输出统一的提醒、数位动作和步骤数据。
- 覆盖整十边界、两位数、多种拆分方式及随机题测试。

##### Phase 2：设置与第一层提醒

- 修改设置页辅助开关说明，明确辅助需要在做题时主动点击。
- 明确 `assistMethod` 只控制退位减法的方法。
...
- 实现十格框、圆点、一个十与十个一之间转换的基础组件。
- 统一进位上箭头、退位下箭头和数位提示条。
- 支持移动、拆分、合并、移除和当前步骤高亮。
- 确保动画可跳过，并尊重系统的减少动态效果设置。

##### Phase 4：三种方法动画

- 实现凑十法动画，并同步展示“10 个一换成 1 个十、向十位进 1”。
- 实现破十法动画，并同步展示“从十位退 1、1 个十换成 10 个一”。
- 实现平十法动画，明确连续减法过程中同样发生了退位。
- 动画结束后不显示可直接复制的最终输入，重新聚焦答案输入框。
...
- 补充 MathAssist 组件交互测试。
- 补充 Session 中辅助入口、切题重置和提交答案流程测试。
- 验证开启和关闭辅助时原有做题流程均不受影响。
- 验证刷新、历史记录、订正和旧数据兼容性。
- 在桌面端检查不同运算范围下的布局和动画表现。

#### v2.4 验收标准

- 不进位、不退位或无法生成有效拆分步骤的题目不出现辅助入口。
- 符合条件的题目只在开启辅助后显示低干扰的“需要提示”按钮。
- 用户未点击时，不显示任何提示或动画。
- 第一层只展示进位或退位关键提醒，不直接给出答案。
- 第二层同时展示计算方法和数位变化，不能只演示凑十、破十或平十。
