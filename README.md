...
在现有错题训练的基础上，新增「加减法训练」模块，覆盖一年级核心的加减法计算能力训练。

### 路由设计

| 路径 | 页面 | 说明 |
|---|---|---|
| `/` | **首页入口** | 改为两个入口卡片：错题列表 / 加减法训练 |
| `/problems` | 错题列表 | 保留不动 |
...
├── components/
│   └── AppLayout/index.js         ← 改造：导航栏加「加减法训练」
├── utils/
│   ├── mathGenerator.js           ← 新增：加减法题目生成器
...
- 导航栏「加减法训练」
- 首页改造为两个入口卡片
- 参数调整页（范围/比例/概率/数量 + 辅助运算开关 + 破十法/平十法）
- `mathGenerator.js` 题目生成器
- `useTimer.js` 计时器
- 基础做题页（无辅助运算，提交→直接下一题）

#### Phase 2：结算 + 错误分析
- `storage.js` 数据存储
- 结算页（分数/用时/逐题批改/错误原因分类）
- 形成完整训练闭环
